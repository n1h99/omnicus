...
Resend webhook и durable receipts, email wait привязан к execution/thread/контакту.
Sender/body/headers фиксируются перед отправкой; UNKNOWN нельзя слепо повторять.
Релиз требует отдельной миграции и внешней настройки Resend/DNS; успешный push
не подтверждает деплой или live-приёмку. Не смешивать это с baseline ниже. Старые cleanup
